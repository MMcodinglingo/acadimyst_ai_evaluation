...
const handleIeltsWritingAiEvaluation = async ({ studentWritingAnswer, student, testData, tasks }) => {
    try {
        ...
        const evaluateSingleTask = async ({ task, taskIndex }) => {
            ...
            // Log Step 1 output to file
            const logsDir = path.join(process.cwd(), 'logs', 'step-outputs');
            if (!fs.existsSync(logsDir)) fs.mkdirSync(logsDir, { recursive: true });
            const step1File = path.join(logsDir, `task${taskNumber}_step1_extract_${Date.now()}.txt`);
            fs.writeFileSync(step1File, JSON.stringify({ extractPrompt, extractResponse }, null, 2));
            ...
            // Log Step 2 output to file
            const step2File = path.join(logsDir, `task${taskNumber}_step2_assess_${Date.now()}.txt`);
            fs.writeFileSync(step2File, JSON.stringify({ assessPrompt, assessResponse }, null, 2));
